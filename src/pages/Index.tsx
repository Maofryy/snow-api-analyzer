...
function BenchmarkDashboard() {
    ...
    const fetchInstanceInfo = useCallback(async () => {
        try {
            ...
            const tokenResponse = await fetch("/api/x_elosa_api_benc_0/app/get-token", {
                method: "GET",
            ...
            const instanceResponse = await fetch("/api/x_elosa_api_benc_0/app/instance-info", {
                method: "GET",
                credentials: "include",
                headers: {
                    "Content-Type": "application/json",
    ...
    const handleRunTestFromSpecs = async (spec: ProcessedTestSpec) => {
        ...
        const categoryKey = (() => {
            switch (spec.category) {
                case "Dot-Walking Performance":
                    return "dotWalkingTests";
                case "Multi-Table Queries":
                    return "multiTableTests";
                case "Schema Tailoring":
                    return "schemaTailoringTests";
                case "Performance at Scale":
                    return "performanceScaleTests";
            ...
            <main className="container mx-auto px-6 py-8">
                ...
                {isProductionMode() && (
                    <Alert className="mb-8">
                        ...
                        <AlertDescription>
                            <strong>Production Mode:</strong> This application is running inside ServiceNow using session authentication. Instance configuration is automatically managed through your
                            ServiceNow session.
                        </AlertDescription>
                    </Alert>
                )}
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    ...
                    <TabsContent value="test-specs" className="mt-0">
                        <TestSpecsExplorer onRunTest={handleRunTestFromSpecs} onSwitchToBenchmark={handleSwitchToBenchmark} />
                    </TabsContent>
                </Tabs>
            </main>
        </div>
